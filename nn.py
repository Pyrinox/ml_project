...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        "*** YOUR CODE HERE ***"
        inputs = []
        for each in node.get_parents():
            inputs.append(self.get_output(each))
        return inputs
    ...
    def add(self, node):
        ...
        gradient = np.zeros(np.shape(output))
        self.node_info[node] = (output, gradient)
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        self.node_info[loss_node] = (self.node_info[loss_node][0], 1.0)

        reverse = reversed(self.nodes)
        for node in reverse:
            parents = node.get_parents()
            backward_calc = node.backward(self.get_inputs(node), self.get_gradient(node))

            for num in range(len(parents)):
                parent = parents[num]
                self.node_info[parent] = (self.node_info[parent][0], self.node_info[parent][1] + backward_calc[num])
...
class Add(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.full(np.shape(inputs[0]), gradient), np.full(np.shape(inputs[1]), gradient)]
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        if len(np.unique(gradient)) > 1:
            return [np.dot(gradient, np.transpose(inputs[1])), np.dot(np.transpose(inputs[0]), gradient)]
        return [np.full(np.shape(np.transpose(inputs[1])), gradient[0][0]) * np.transpose(inputs[1]), np.transpose(inputs[0]) * np.full(np.shape(np.transpose(inputs[0])), gradient[0][0])]
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        if len(np.unique(gradient)) > 1:
            return [np.ones(np.shape(inputs[0])) * gradient, np.sum(np.ones(np.shape(inputs[1])) * gradient, axis=0)]
        return [np.ones(np.shape(inputs[0])), np.ones(np.shape(inputs[1]))]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        a = np.where(inputs[0] > 0, 1, inputs[0])
        b = np.where(a < 0, 0, a)
        return [b * gradient]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [((inputs[0] - inputs[1]) * gradient)/np.shape(inputs[0])[0], ((inputs[0] - inputs[1]) * gradient)/-np.shape(inputs[0])[0]]
