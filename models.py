...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.hlsize = 100
        self.w1 = nn.Variable(1, self.hlsize)
        self.w2 = nn.Variable(self.hlsize, 1)
        self.b1 = nn.Variable(self.hlsize)
    ...
    def run(self, x, y=None):
        ...
        mult1 = nn.MatrixMultiply(graph, input_x, self.w1)
        add1 = nn.MatrixVectorAdd(graph, mult1, self.b1)
        relu1 = nn.ReLU(graph, add1)
        mult2 = nn.MatrixMultiply(graph, relu1, self.w2)
        ...
        if y is not None:
            ...
            loss1 = nn.SquareLoss(graph, add2, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            # print(np.shape(graph.get_output(graph.get_nodes()[-1])))
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        # .18
        self.learning_rate = .21
        self.hlsize = 230
        self.w1 = nn.Variable(784, self.hlsize)
        self.w2 = nn.Variable(self.hlsize, self.hlsize)
        self.w3 = nn.Variable(self.hlsize, 10)
        self.b1 = nn.Variable(self.hlsize)
        self.b2 = nn.Variable(self.hlsize)
    ...
    def run(self, x, y=None):
        ...
        mult3 = nn.MatrixMultiply(graph, relu2, self.w3)

        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss1 = nn.SoftmaxLoss(graph, mult3, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(graph.get_nodes()[-1])
...
class DeepQModel(Model):
    ...
    def __init__(self):
        ...
        # You may use any learning rate that works well for your architecture
        "*** YOUR CODE HERE ***"
        self.learning_rate = .0058
        self.hlsize = 100
        self.w1 = nn.Variable(4, self.hlsize)
        self.b1 = nn.Variable(self.hlsize)
        self.w2 = nn.Variable(self.hlsize, 2)
    ...
    def run(self, states, Q_target=None):
        ...
        mult1 = nn.MatrixMultiply(graph, input_states, self.w1)
        add1 = nn.MatrixVectorAdd(graph, mult1, self.b1)
        relu1 = nn.ReLU(graph, add1)
        mult2 = nn.MatrixMultiply(graph, relu1, self.w2)
        add2 = nn.MatrixVectorAdd(graph, mult2, self.b2)
        ...
        if Q_target is not None:
            ...
            loss1 = nn.SquareLoss(graph, input_Q_target, add2)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(graph.get_nodes()[-1])
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        self.d = 100
        self.w1 = nn.Variable(self.d, self.d)
        self.w2 = nn.Variable(self.d, 5)
        self.b1 = nn.Variable(self.d)
        self.b2 = nn.Variable(5)
        self.reshape = nn.Variable(47, self.d)
        self.reshape2 = nn.Variable(self.d, self.d)
    ...
    def run(self, xs, y=None):
        ...
        "*** YOUR CODE HERE ***"
        graph = nn.Graph([self.w1, self.w2, self.b1, self.b2, self.reshape, self.reshape2])
        h = nn.Input(graph, np.zeros((batch_size, self.d)))

        for x in xs:
            input_x = nn.Input(graph, x)
            construct_mult1 = nn.MatrixMultiply(graph, input_x, self.reshape)
            construct_mult2 = nn.MatrixMultiply(graph, h, self.reshape2)
            b = nn.Add(graph, construct_mult1, construct_mult2)
            h = nn.ReLU(graph, b)

        mult1 = nn.MatrixMultiply(graph, h, self.w1)
        add1 = nn.MatrixVectorAdd(graph, mult1, self.b1)
        relu1 = nn.ReLU(graph, add1)
        mult2 = nn.MatrixMultiply(graph, relu1, self.w2)
        add2 = nn.MatrixVectorAdd(graph, mult2, self.b2)
        ...
        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss1 = nn.SoftmaxLoss(graph, add2, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(graph.get_nodes()[-1])